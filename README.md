...
8. **Firefly image (`firefly`)**
   ...
   - **What we add on top of upstream**: the vendored `cadc-sso/` Gradle multi-project (sourced from [`opencadc/science-containers`](https://github.com/opencadc/science-containers/tree/main/science-containers/Dockerfiles/firefly/cadc-sso), Java 21) builds a `cadc-sso-lib-*.jar` that implements Firefly's `sso.framework.adapter` extension point. We carry three minimal local edits on top of the upstream sources to track Firefly's Tomcat 9 → Tomcat 11 migration in the 2025.x line: bumping `def fireflyTag` to `release-2025.5.4`, swapping `javax.servlet:javax.servlet-api:4.0.1` for `jakarta.servlet:jakarta.servlet-api:6.1.0` in `lib/build.gradle`, and switching `TokenRelay.java` (and its test) to `import jakarta.servlet.http.Cookie`. Without those three, the plugin throws `NoSuchMethodError` at runtime inside `ipac/firefly:2025.5` because `RequestAgent.getCookie()` returns `jakarta.servlet.http.Cookie` since 2025.x. **Invariant: `FIREFLY_VERSION` (Dockerfile) and `fireflyTag` (`lib/build.gradle`) MUST stay in the same Firefly major-minor line; merge their Renovate PRs together.** The `org.opencadc.security.sso.TokenRelay` class reads the `CADC_SSO` cookie from inbound requests and converts it into a `Bearer <token>` header for Firefly's outbound calls to CADC services (YouCAT, VOSpace, etc.) so users see authenticated data. The plugin is dropped into Tomcat's `webapps-ref/firefly/WEB-INF/lib/` and activated at runtime by setting `PROPS_sso__framework__adapter=org.opencadc.security.sso.TokenRelay` in the deployment manifest.
